  ...
  for (p = proc; p < &proc[NPROC]; p++) {
    ...
    if (p->state == RUNNABLE || p->state == RUNNING) {
      ...
      if (min_vruntime == -1) {
        ...
        if (state_vruntime < min_vruntime) {
          min_vruntime = state_vruntime;
          min_proc = p;
        }
      }
...
// Writes into addr cfs_priority, runtime, runnable time and sleeptime
// of the process with given pid
// Return -1 if this process was not found.
int
